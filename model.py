...
def att_emb_ngram_encoder_maxout_keywords(x_emb, x_mask, G, seq_len, opt):
    x_mask = tf.expand_dims(x_mask, axis=-1) # b * s * 1
    x_emb_0 = tf.squeeze(x_emb,) # b * s * e
    ...
    G_conv1 = tf.contrib.layers.conv2d(Att_v, num_outputs=opt.embed_size, kernel_size=[opt.ngram], padding='SAME',
                                     activation_fn=tf.nn.relu)

    x_grue = BidirectionalGRUEncoder(x_emb_0, seq_len, opt)#b*s*his*2

    Att_v = tf.reduce_max(Att_v, axis=-1, keep_dims=True)
    Att_v_max = partial_softmax(Att_v, x_mask, 1, 'Att_v_max') # b * s * 1

    G_v1 = tf.reduce_max(G_conv1, axis=-1, keep_dims=True)
    G_v2 = tf.squeeze(G_v1)
    G_conv2 = tf.contrib.layers.fully_connected(G_v2, num_outputs=opt.embed_size, activation_fn=tf.nn.relu)

    x_att = tf.multiply(x_grue, Att_v_max)
    H_enc = tf.reduce_sum(x_att, axis=1)

    H_enc2 = tf.contrib.layers.fully_connected(H_enc, num_outputs=opt.embed_size, activation_fn=tf.nn.relu)
    return H_enc2, G_conv2

def BidirectionalGRUEncoder(inputs, sequence_length, opt):
    #输入inputs的shape是[batch_size, max_time, voc_size]
    with tf.variable_scope("GRUE"):
        GRU_cell_fw = rnn.GRUCell(opt.H_dis)
        GRU_cell_bw = rnn.GRUCell(opt.H_dis)
