...
def prepare_data_for_emb(seqs_x, G, opt):
    maxlen = opt.maxlen
    ...
    if maxlen != None:
        ...
        new_lengths_x = []
        new_G = []
        for l_x, s_x, g_x in zip(lengths_x, seqs_x, G):
            if l_x < maxlen:
    ...
    # 用0补全，使得这一batch中的句子长度相等
    n_samples = len(seqs_x)
    x = np.zeros((n_samples, maxlen)).astype('int32')
    x_mask = np.zeros((n_samples, maxlen)).astype('float32')
    G_G = np.zeros((n_samples, maxlen, opt.num_class)).astype('float32')

    for idx, s_x in enumerate(seqs_x):
        x[idx, :lengths_x[idx]] = s_x
        G_G[idx, :lengths_x[idx]] = G[idx]
        x_mask[idx, :lengths_x[idx]] = 1.  # change to remove the real END token

    return x, x_mask, G_G, lengths_x
